...
def run_detection(model_name:str, data_path, cross_data_path, part):
    ...
    for ix, val in enumerate(val_idx):
        ...
        cv2.imwrite(f'result/pretrained/PAMaskRcnnalldatapred/{val}', v.get_image()[:, :, ::-1])
        ...
        result_name = 'results_' + 'PAMaskRcnnalldata'
        Path(f'result/pretrained/{result_name}/').mkdir(parents=True, exist_ok=True)
...
if __name__ == '__main__':
    ...
    total_time = 0
    for i in range(0, 4):
        total_time += run_detection('checkpoints/PAMaskRcnnalldata', data_path=data_path, cross_data_path=cross_data_path, part=i)

    print(f'Total time:{total_time}')
    print(f'Inference_time:{total_time/316}')
